...
using System.Net;
using WeaponsService.Contracts;

namespace WeaponsService.Handlers
{
    /// <summary>
    ...
    public class WeaponHandler
    {
        ...
        public bool ReduceWeapon(long id)
        {
            ...
            weapon.Quantity--;
            Console.WriteLine($"Reduce 1 {weapon.Title}");
            return true;
        }
        ...
        public async void AddWeapon(long id, int quantity)
        {
            ...
            try
            {
                using (var producer = new ProducerBuilder<Null, string>(config).Build())
                {
                    ...
                    var result = await producer.ProduceAsync($"Service1.Add.Weapon{weapon.Id}", new Message<Null, string> { Value = $"{weapon.Quantity}" });
